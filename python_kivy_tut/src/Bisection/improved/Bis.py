...
class Screen2(Screen):
    def pop_tab(self):
        ...
        f=c.fetchall()
        self.ids.gd.add_widget(Label(text='Lower lim',color=[1,1,0,1]))
        self.ids.gd.add_widget(Label(text='Upper lim',color=[1,1,0,1]))
        self.ids.gd.add_widget(Label(text='Mid point',color=[1,1,0,1]))
        self.ids.gd.add_widget(Label(text='f(x)',color=[1,1,0,1]))
        colr1=[.3,.9,.8,1]
        colr2=[.4,1,.5,1]
        count=True
        for row in f:
            if count==True:
                ...
                count=True
            for cols in row:
                t=TextInput(text=str(cols),background_color=colr)
